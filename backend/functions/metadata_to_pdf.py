...
def _equalize_lists(list1, list2):
...
def create_dpp_pdf(
    ...
    *,
    ...
    if include_answer_key:
        questions,answer_key = _equalize_lists(questions,answer_key)

        # Equalize first
        questions, answer_key = _equalize_lists(questions, answer_key)

        # Slice or pad questions
        questions = questions[:total_q]
        if len(questions) < total_q:
            questions += [""] * (total_q - len(questions))

        # Remove empty questions AFTER padding
        original_count = len(questions)
        questions = [q for q in questions if q]

        # Adjust answer_key to match final question list
        if answer_key:
            # Trim or pad answer_key to match current questions list
            answer_key = answer_key[:original_count]
            answer_key = [answer_key[i] if i < len(answer_key) else "[Answer]" for i in range(len(questions))]
    ...
    md_lines += [
        "",
        "---",
        f'<div class="footer">{footer_text} • {datetime.now():%Y-%m-%d %H:%M:%S}</div>',
    ]
