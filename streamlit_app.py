...
def main_application():
    ...
    with st.form("pdf_form", clear_on_submit=False):
        ...
        with col6:
            include_answer_key = st.selectbox("📝 Include answer key?", ["yes", "no"], index=1, help="Choose if you want to include answer key or not")

        # Fixed: col7 should be on the next line, not in the side
        col7 = st.columns(1)[0]  # Get the first (and only) column from the list
        with col7:
    ...
    if generate_button:
        ...
        try:
            ...
            pdf_path = DPPify().run(
                ...
                answer_key=(include_answer_key=="yes") #return True if include_answer_key=="yes" else return False
            )
